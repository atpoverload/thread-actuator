...
public class ActuatedBucketRenderer implements ImageSampler {
  private static BitSet setupActuation() {
    int[] frequencies =
        Arrays.stream(System.getProperty("clerk.actuation.frequencies").split(","))
            .mapToInt(Integer::parseInt)
            .toArray();
    BitSet affinity = new BitSet(Runtime.getRuntime().availableProcessors());
    for (int cpu = 0; cpu < frequencies.length; cpu++) {
      int frequency = frequencies[cpu];
      if (frequency != 0) {
  ...
  public void render(Display display) {
    ...
    BitSet affinity = setupActuation();
