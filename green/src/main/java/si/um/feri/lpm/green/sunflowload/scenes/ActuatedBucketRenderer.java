package si.um.feri.lpm.green.sunflowload.scenes;
...
public final class ActuatedBucketRenderer implements ImageSampler {
  static final String ACTUATION_PARAMETERS = "actuation";

  // encoding for a map of integer cpu -> integer frequency in kilohertz:
  // 0,1200000;1,1300000;4,2100000;...
  // all entries are assumed to be
  static String toActuationString(Map<Integer, Integer> actuation) {
    return actuation
        .entrySet()
        .stream()
  ...
  static Map<Integer, Integer> parseActuationString(String actuationString) {
    HashMap<Integer, Integer> actuation = new HashMap<>();
    for (String paramsString : actuationString.split(";")) {
      String[] params = paramsString.split(",");
      actuation.put(Integer.parseInt(params[0]), Integer.parseInt(params[1]));
    }
    return actuation;
  }

  // setup the actuation by scaling the listed cpus and adding them to a bitset for affinity setting
  ...
  private Map<Integer, Integer> actuation = new HashMap<>();

  public ActuatedBucketRenderer() {
    bucketSize = 32;
  ...
  public boolean prepare(Options options, Scene scene, int w, int h) {
    ...
    // pull out the actuation parameters
    String actuationString = options.getString(ACTUATION_PARAMETERS, "");
    if (!actuationString.isEmpty()) {
      actuation = parseActuationString(actuationString);
    }
  ...
  public void render(Display display) {
    ...
    // setup the actuation and pipe forward the affinity
    BitSet affinity = setupActuation(actuation);

    for (int i = 0; i < renderThreads.length; i++) {
      renderThreads[i] = new BucketThread(i, affinity);
    ...
    // make sure to reset after each run
    Dvfs.reset();
    UI.taskStop();
    timer.end();
  ...
  private class BucketThread extends Thread {
    ...
    BucketThread(int threadID, BitSet affinity) {
    ...
    @Override
    public void run() {
      // the thread's affinity must be set when it is running, so it has to happen here
      Affinity.setAffinity(affinity);
      while (true) {
        int bx, by;
        synchronized (ActuatedBucketRenderer.this) {
